...
class YoloPredictor(BasePredictor, QObject):
    ...
    def run(self,
            ...
            # data='industry.yaml',  # dataset.yaml path
        ...
        source = self.source if self.source is not None else self.args.source
        ...
        self.vid_path, self.vid_writer = vid_path, vid_writer
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def open_src_file(self):
        ...
        cata=0
        print(open_fold)
        if 'daily' in self.Model_name.text():  # If the model is not selected, the default model is used
            cata=1
        else:
            cata=0
        print('-'*10)
        open_fold = open_fold[cata]
        print(open_fold)
        if not os.path.exists(open_fold):
            open_fold = os.getcwd()
        name, _ = QFileDialog.getOpenFileName(self, 'Video/image', open_fold, "Pic File(*.mp4 *.mkv *jpeg *.avi *.flv *.jpg *.png)")
        if name:
            self.yolo_predict.source = name
            self.show_status('Load File：{}'.format(os.path.basename(name))) 
            config['open_fold'][cata] = os.path.dirname(name)
